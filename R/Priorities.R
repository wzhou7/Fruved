Score_Priorities <- function(data){
  ...
  for(i in 1:5){
    if(dh[1,index[i]]>0){
      scores[3*(i-1)+1] <- map_priorities[,2][map_priorities[,1]==paste0("PRIORTIES_",index[i])]
  ...
  if(!is.na(dh[2,all_index[5]]) & !is.na(dh[2,all_index[6]])){
    if(dh[2, all_index[5]] == dh[2, all_index[6]]){
      scores[16] <- map_priorities[,2][map_priorities[,1]==paste0("PRIORTIES_",all_index[6])]
      for(j in 7:length(all_index)){
        if(!is.na(dh[2,all_index[j]]) & (dh[2,all_index[j]]==dh[2,all_index[5]])){
          scores[16] <- paste(scores[16],
                              map_priorities[,2][map_priorities[,1]==paste0("PRIORTIES_",all_index[j])],sep=" AND ")
        }
      }
    }
  }
