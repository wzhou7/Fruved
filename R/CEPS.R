Score_CEPS <- function(dta){
  ind1 <- which(names(dta)=="CEPSexclass")
  ind2 <- which(names(dta)=="CEPSopppa")
  vars <- names(dta)[ind1:ind2]
  data <- dta[,vars]
...
# #test
# out1 <- Score_CEPS(data2)
# out2 <- data2[,c("SC1_0","SC4_0","SC7_0","SC8_0","SC9_0","SC10_0","SC11_0")]
# for(j in 1:ncol(out2)){
#   out2[,j][out2[,j]<0] <- NA
# }
# x <- y <- c()
# for(j in 1:ncol(out2)){
#   x[j] <- sum(out1[,j]!=out2[,j],na.rm=TRUE)
#   y[j] <- sum(is.na(out1[,j])) - sum(is.na(out2[,j]))
# }
# x
# y